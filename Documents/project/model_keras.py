        ...
        for i in models:
            ...
            for n in range(0, n_epochs, n_steps):
                ...
                (n+n_steps,
                 ...
                 quadratic_weighted_kappa.quadratic_weighted_kappa(labels_test, y_pred)))
            ...
            quadratic_weighted_kappa.quadratic_weighted_kappa(labels_test, fitted)))
...
def model_factory(n_classes, n_dims, net_type):
    ...
    model = Sequential()
    model.add(Dense(layer1_sz, input_dim=n_dims, kernel_initializer='glorot_uniform'))
    model.add(Activation('relu'))
#    model.add(Dropout(0.5))

    model.add(Dense(layer2_sz, input_dim=layer1_sz, kernel_initializer='glorot_uniform'))
    model.add(Activation('relu'))
#    model.add(Dropout(0.5))
    
    model.add(Dense(n_classes, input_dim=layer2_sz, kernel_initializer='glorot_uniform'))
